...
// --- Helper Functions (pindahkan ke sini atau ke file service/utility jika ada) ---
const savetube = {
    api: {
...
const ytConvert = {
    ...
    async download(youtubeUrl, userFormat = 'audio') {
        ...
        const payload = eval("(" + m1 + ")"); // HATI-HATI DENGAN eval()
...
const ytmp3mobi = async (youtubeUrl, format = "mp4") => {
    ...
    while (progress != 3) {
        ...
        await new Promise(re => setTimeout(re, 1000)); // Tambahkan delay agar tidak terlalu cepat
    }
...
export default {
    ...
    async run(req, res) {
        try {
            ...
            const result = await ytConvert.download(url, "audio");
            ...
            console.error("[YouTube MP3 Downloader Error]", err);
            res.status(500).json({
                statusCode: 500,
                success: false,
                creator: "GIMI❤️",
                error: err.message || "Failed to download YouTube MP3.",
            });
        }
    },
};
